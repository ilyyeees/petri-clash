...
def main():
    ...
    out_path = Path(args.out) if args.out else Path("weights") / f"{target_path.stem}.pt"
    device = args.device
    ...
    for step in range(1, args.steps + 1):
        batch_ids = torch.randint(args.pool_size, (args.batch_size,), device=device)
        batch = pool[batch_ids].clone()

        # one fresh seed in the batch keeps the organism honest
        batch[0] = fresh_seed

        rollout = random.randint(args.min_rollout, args.max_rollout)
        ...
        if step % 50 == 0 or step == 1 or step == args.steps:
            ...
            speed = step / elapsed
            print(
                f"step {step:4d}/{args.steps} loss {value:.5f} ema {ema:.5f} "
